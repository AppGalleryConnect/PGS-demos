...
export class DisplayManager extends Component {
    ...
    public updateState(deltaTime: number, gameState: GameState, isCheck:boolean) {
        ...
        // for(let i = props.length; i < keyArray.length; i++) {
        //     if (this._dicProps[i]) {
    ...
    private _onGameEnd () {
        ...
        if(players[winner].channel.openId == GobeUtil.instance.ownPlayerId){
            if(PlayerData.instance.playerInfo["score"] < players[winner].score){
                PlayerData.instance.updatePlayerInfo("score", players[winner].score);
                GobeUtil.instance.inputRank();
            }
        }
        ...
        players.forEach((value: Player, index: number) => {
            if (value.channel) {
                ...
                ResourceUtil.loadModelRes(playerPath).then((pf: any)=>{
                    ...
                    ndPlayer.getComponent(Fighter)?.init(this);
