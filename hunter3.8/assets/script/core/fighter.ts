import { _decorator, Component, Node, SkinningModelComponent, ParticleSystemComponent, Vec3, Mat4 } from 'cc';
...
export class Fighter extends Component {
    ...
    private _playType:PlayType = PlayType.idle;

    public init(parent: DisplayManager) {
        this._parent = parent;
        this._isPlayRunningEffect = false;
        this._runEffect = null!;
        this._score = 0;

        this.fighterModel.playAni(Constant.ANI_TYPE.IDLE, true, false, ()=>{}, 8);
    ...
    */
    ...
    public playMove(logicManager: LogicManager, dt:number, otherNode:Node, player:Player){
        ...
        this.vec3_pos.x += leftX * 0.2 * dt * 60;
        this.vec3_pos.z -= leftY * 0.2 * dt * 60;
        ...
        if(this._moveIndex == 0){
            GobeUtil.instance.sendFrame({'A': Constant.ACTION.MOVE, 'V': this.vec3_angle.y, 'X': this.vec3_lastPos.x, 'Z': this.vec3_lastPos.z});
        }
     ...
     */
     public playMoveAi(logicManager: LogicManager, dt:number, otherNode:Node, player:Player){
        ...
        var leftX:number = PlayerData.instance.getEasyTouchInfo(Constant.EASY_TOUCH.TOUCH_LEFT_X) * -1;
        var leftY:number = PlayerData.instance.getEasyTouchInfo(Constant.EASY_TOUCH.TOUCH_LEFT_Y) * -1;
        if (leftY == 0 && leftX == 0) {
            leftX = this._leftX;
            leftY = this._leftY;
        }else{
    ...
    public updateState (deltaTime: number, value: Player, isCheck:boolean) {
        ...
        if(isCheck || value.channel.openId != GobeUtil.instance.ownPlayerId && !GobeUtil.instance.isAi){
            ...
            if(!value.position.equals(this.node.position)){
